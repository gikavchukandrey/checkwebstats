...
class Dompdf
{
    ...
    */
    ...
    private function processHtml()
    {
        ...
        foreach ($stylesheets as $tag) {
            switch (strtolower($tag->nodeName)) {
                ...
                case "link":
                    if (strtolower(stripos($tag->getAttribute("rel"), "stylesheet") !== false) || // may be "appendix stylesheet"
                        ...
                        if (count($formedialist) > 0) {
                            ...
                            foreach ($formedialist as $type) {
                                if (in_array(strtolower(trim($type)), $acceptedmedia)) {
                                    $accept = true;
                                    break;
                                }
                            }
    ...
    public function getPaperSize($paperSize = null)
    {
        ...
        if (is_array($size)) {
            return $size;
        } else if (isset(Adapter\CPDF::$PAPER_SIZES[strtolower($size)])) {
            return Adapter\CPDF::$PAPER_SIZES[strtolower($size)];
        } else {
