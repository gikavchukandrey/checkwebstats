		...
		switch ($action) {
			...
			case  config_item("slug_filter"):	
				...
				if($seg2 == config_item('slug_made_with'))
				{
					...
					$this->DATA['list'] 	= $data->result();
					$this->DATA['title']		= __("Filter sites by ").__(ucfirst(strtolower($seg2_text))).": <img  style='vertical-align: text-top ;height:40px;' src='".base_url()."assets/images/icons/".$this->DATA['list'][0]->icon."'> ".$seg3." - ".__("Page ").$seg4;
				...
				else
				{
					...
					$this->DATA['list'] 	= $data->result();
					$this->DATA['title'] 		= __("Filter sites by ").__(ucfirst(strtolower($seg2_text))).": ".$seg3." - ".__("Page ").$seg4;	

				}

				$this->DATA['SEO']["title"] 		= __("Filter sites by ").__(ucfirst(strtolower($seg2_text))).": ".$seg3." - ".__("Page ").$seg4;		
				...
				$this->load->helper('seo');				
				 ...
				 else
				 {
					...
					if(!$site_data)
					{
					 ...
					 $this->DATA['SEO']["title"] 		= ucfirst(strtolower($site_data->url))." ".__("- SEO Checker - Website Review");
					 $this->DATA['SEO']["description"]  = ucfirst(strtolower($site_data->url))." ".__(" - Website Review, SEO, Estimation Traffic and Earnings And Speed And Optimization Tips ");
