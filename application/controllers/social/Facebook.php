...
public $client_secret = "";
 function __construct()
	...
	{	
		...
		if($code)
		{			
			$token 	= json_decode(_curl("https://graph.facebook.com/v2.8/oauth/access_token?client_id=".$this->client_id."&redirect_uri=".$this->redirect_uri."&client_secret=".$this->client_secret."&code=$code"));									
			...
			{	
				if($profile->email == '')
				{
					show_error("Error Facebook: Su cuenta de facebook no tiene permisos para ingresar",403);
					exit;
			...
			if($profile->id == '')
				show_error("Error 1154",403);

			if($exist)
			{
				$save['bio'] 	= $profile->about;	
				if(!$save['bio'])
					unset($save['bio']);
	...
	{		
		...
		return _curl("https://graph.facebook.com/v2.8/me?fields=email,name,about,cover&access_token=".$token."&appsecret_proof=".$appsecret_proof);
	}
}